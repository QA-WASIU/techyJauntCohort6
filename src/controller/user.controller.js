...
const signup = async (req, res) => {
  const { firstName, lastName, identifier, password, verifyPassword } = req.body;
  // Validate input
  if (!firstName || !lastName || !identifier || !password || !verifyPassword) {
    return res.status(400).json({ message: "All fields are required" });
  }
  // Validate identifier (phone or email)
  if (!(isValidEmail(identifier) || isValidPhone(identifier))) {
    return res.status(400).json({ message: "Identifier must be a valid email or 11-digit phone number." });
  }
  ...
  try {
    ...
    const welcomeTemplate = emailTemplates.welcomeTemplate(firstName, lastName, emailToken);
...
const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    user.otp = null; // Clear OTP after verificationa
    await user.save();
